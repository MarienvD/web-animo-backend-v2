...
//            model.setMapCytoscapeIDtoReactionID(edgeSUIDToModelId);
    ...
    public static Model getModelFromJson(CytoscapeModel object, Integer nMinutesToSimulate) throws AnimoException, IllegalAccessException {
        ...
        Map<String, Long> nodeJSONIDtoSUID = new HashMap();
        Map<String, Long> edgeJSONIDtoSUID = new HashMap();
        NetworkData modelData = object.getData();

        List<Method> methods = Arrays.stream(NetworkData.class.getDeclaredMethods())
        ...
        for(Field field : NetworkData.class.getDeclaredFields()) {
            try {
                ...
                if (foundGetMethod.isPresent()) {
                    model.getProperties().let(field.getName()).setValue(foundGetMethod.get().invoke(modelData));
                }
        ...
        GraphModel.Elements elements = object.getElements();
        HashMap<String, String> nameToID = new HashMap();
        ...
        Collections.sort(nodesList, new Comparator<GraphModel.Node>() {
            public int compare(GraphModel.Node n1, GraphModel.Node n2) {
                GraphModel.NodeData node1Data = n1.getData();
                GraphModel.NodeData node2Data = n2.getData();

                String name1 = Optional.ofNullable(node1Data.getCanonicalName()).or(() -> Optional.ofNullable(node1Data.getName())).orElse("");
                String name2 = Optional.ofNullable(node2Data.getCanonicalName()).or(() -> Optional.ofNullable(node2Data.getName())).orElse("");
                return name1.compareTo(name2);
            }
        });
        ...
        for (GraphModel.Node node : nodes) {
            GraphModel.NodeData data = node.getData();
            ...
            boolean plotted = data.isPlotted();
            if (plotted && enabled) {
                noReactantsPlotted = false;
            }
            ...
            r.let("enabled").be(enabled);
            r.let("plotted").be(plotted);
            r.let("canonicalName").be(Optional.of(data.getCanonicalName()).or(() -> Optional.ofNullable(data.getName())).orElse(""));
            r.let("alias").be(r.get("canonicalName").as(String.class));
            int nLevels = Optional.ofNullable(data.getLevels()).orElse(100);
            if (nLevels > maxNumberOfLevels) {
                maxNumberOfLevels = nLevels;
            }

            r.let("levels").be(nLevels);
            r.let("initialConcentration").be(Optional.of(data.getInitialConcentration()).orElse(0));
...
//            }
            ...
            for(int i = 0; i < edges.size(); ++i) {
                ...
                r.let("enabled").be(enabled);
                String min1 = "-1";
                String sourceID = Optional.ofNullable(data.getSource()).orElse(min1);
                String targetID = Optional.ofNullable(data.getTarget()).orElse(min1);
                if (enabled && !sourceID.equals(min1) && !targetID.equals(min1)) {
                    if (nodeSUIDToModelId.get(nodeJSONIDtoSUID.get(sourceID)) != null && nodeSUIDToModelId.get(nodeJSONIDtoSUID.get(targetID)) != null) {
                        Reactant sourceNode = model.getReactant(nodeSUIDToModelId.get(nodeJSONIDtoSUID.get(sourceID)));
                        Reactant targetNode = model.getReactant(nodeSUIDToModelId.get(nodeJSONIDtoSUID.get(targetID)));
                        if (sourceNode != null && targetNode != null) {
                            int increment = Optional.ofNullable(data.getIncrement()).orElse(0);
                            if (increment == 0) {
                                if (Long.valueOf(sourceID).equals(Long.valueOf(targetID))) {
                                    increment = -1;
                                } else {
                                    increment = 1;
                                }
                            ...
                            r.let("increment").be(increment);
                            int scenarioIdx = Optional.of(data.getScenario()).orElse(0);
                            r.let("scenario").be(scenarioIdx);
                            if (scenarioIdx == 2) {
                                r.let("_REACTANT_E1").be(Optional.of(data.getReactantE1()).orElse(sourceID));
                                r.let("_REACTANT_ACT_E1").be(data.isReactantE1Active());
                                r.let("_REACTANT_E2").be(Optional.of(data.getReactantE2()).orElse(targetID));
                                r.let("_REACTANT_ACT_E2").be(data.isReactantE2Active());
                            }

                            String sourceName = sourceNode.get("canonicalName").as(String.class);
                            String targetName = targetNode.get("canonicalName").as(String.class);
                            String edgeName = Optional.ofNullable(data.getCanonicalName()).orElse("");
                            if (edgeName.equals("")) {
                                if (!sourceName.equals("") && !targetName.equals("")) {
                                    StringBuilder nameBuilder = new StringBuilder();
...
//            model.setMapCytoscapeIDtoReactionID(edgeSUIDToModelId);
    ...
    private static Optional<Double> parseDouble(String param) {
        try {
