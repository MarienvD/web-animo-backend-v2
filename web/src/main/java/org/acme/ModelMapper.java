...
public class ModelMapper {
    ...
    public static Model getModelFromJson(JSONObject object, Integer nMinutesToSimulate) throws JSONException, AnimoException {
        ...
        Double secondsPerPoint = modelData.optDouble("seconds per point", 1);
...
//            model.setMapCytoscapeIDtoReactionID(edgeSUIDToModelId);
    ...
    public static Model getModelFromJson(CytoscapeModel object, Integer nMinutesToSimulate) throws AnimoException, IllegalAccessException {
        try {
            String modelString = new ObjectMapper().writeValueAsString(object);
            return getModelFromJson(new JSONObject(modelString), nMinutesToSimulate);
        } catch (JsonProcessingException | JSONException e) {
            log.error("Error while serializing model to JSON", e);
